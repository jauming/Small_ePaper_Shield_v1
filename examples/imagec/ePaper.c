...
void ePaper_begin(EPD_size sz)
{
    ...
    this->DISP_LEN    = this->SIZE_LEN;
    this->DISP_WIDTH  = this->SIZE_WIDTH;
    
    EPD.begin(this->size); //ok
    this->init_io(); //hal
} //ePaper_begin->EPD.begin->ePaper_init_io

void ePaper_spi_detach()
{
}
/*********************************************************************************************************
** Function name:           begin
...
{
    EPD.start(); //hal                // power up the EPD panel
    
    //int tmp = getTemperature();
    //Serial.print("temperature: ");
    EPD.setFactor(25);//getTemperature()); //ok        // adjust for current temperature
}
...
#endif
//ePaper_start->EPD.image->ePaper_end
void ePaper_image_flash(PROGMEM const unsigned char *image)           // read image from flash
{
    this->start();
    EPD.image(image);
    this->end();
